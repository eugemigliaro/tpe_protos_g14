/**
 * access_log.c - registro de accesos por usuario (RF8).
 *
 * Las entradas se guardan en un buffer circular en RAM (para GET_LOG) y
 * se persisten a un archivo de texto en modo append (para sobrevivir
 * reinicios). El formato de cada línea es:
...
static int  recent_count = 0;  /* entradas válidas (satura en LOG_RECENT_MAX) */
...
void
access_log_entry(const char *username, const char *dest, const char *result)
{
...
int
access_log_get_recent(const char **entries, int max_entries)
{
